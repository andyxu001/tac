...
$.fn.html5uploader = function(opts){
	...
	var defaults = {
		...
		url:'',//文件提交的地址
		auto:false,//自动上传
		fileSizeLimit:102400,//文件大小限制，默认100K
		multi:true,//默认允许选择多个文件
		buttonText:'选择文件',//上传按钮上的文字
		removeTimeout: 1000,//上传完成后进度条的消失时间
		itemTemplate:'<li id="${fileID}file"><span style="padding:4px 10px 4px 0;background-color:#DDDDDD;"><span class="filename">${fileName}</span><span class="progressnum">${fileSize}</span></span></li>',//上传队列显示的模板,最外层标签使用<li>
	...
	this.each(function(){
		...
		//创建文件对象
			  ...
			  onSelect: option.onSelect||function(files){
				 ...
				 for(var i=0;i<files.length;i++){
					...
					if(file.size > option.fileSizeLimit){
						alert('文件：“' + file.name + "”（" + formatFileSize(file.size) + "）超过了文件上传大小限制（"+ formatFileSize(option.fileSizeLimit) +"）");
						$('#'+file.index+'file>span').attr('style','padding:4px 10px 4px 0;background-color:#FF3366;');
						$('#'+file.index+'file>span>span').attr('style','color:#FFFFFF;');
						$('#fileValid').val(0);
					}
					//验证文件名（不能带"%","&","="）
					var ex = /[%&=]+/;
					if(ex.test(file.name)){
						alert('文件：“' + file.name + "”（" + formatFileSize(file.size) + "）名称中包含非法字符（% & =）");
						$('#'+file.index+'file>span').attr('style','padding:4px 10px 4px 0;background-color:#FF3366;');
						$('#'+file.index+'file>span>span').attr('style','color:#FFFFFF;');
						$('#fileValid').val(0);
					}
					//判断是否是自动上传
					 if(option.auto && !ex.test(file.name) && file.size <= option.fileSizeLimit){
			  ...
			  onProgress: function(file, loaded, total,progressId) {
				  if(option.onProgress!=null){
					  option.onProgress(loaded,total,progressId);
				  }else{
			  ...
			  funUploadFile: function(file) {
				  ...
				  (function(file) {
					  ...
					  var progressId = new Date().getTime();
					  if (xhr.upload) {
						  // 上传中
						  xhr.upload.addEventListener("progress", function(e) {
							  self.onProgress(file, e.loaded, e.total,progressId);
						  }, false);
						  ...
						  xhr.onreadystatechange = function(e) {
							  if (xhr.readyState == 4) {
								  if (xhr.status == 200) {
									  self.onUploadSuccess(xhr.responseText,progressId);
									  setTimeout(function(){ZXXFILE.onDelete(file.index);},option.removeTimeout);
									 
									  self.onUploadComplete(progressId);	
									  
								  } else {
									  self.onUploadError(progressId);		
								  }
							  }
						  };
			  
			  			  option.onUploadStart(file.name,progressId,formatFileSize(file.size));	
						  // 开始上传
						  xhr.open("POST", self.url, true);	
						  xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");
						  var form = new FormData();
						  form.append("fileitem", file);    						  
						  xhr.send(form);
					  }	
				  })(file);	
